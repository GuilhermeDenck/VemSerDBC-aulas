...
const validarLogin = () => {
  ...
  axios.get(`http://localhost:3000/colaboradores?email=${emailDigitado}`) //Query Parameter
  .then((result) => {
...
const cadastrarUsuario = (event) => {
  ...
  const nomeCompleto = nome.split(' ').map( nome => nome.charAt(0).toUpperCase() + nome.slice(1).toLowerCase() ).join(' ');
